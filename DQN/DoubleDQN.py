...
class DDQNAgent:
    ...
    def update(self):
        ...
        # 采用策略网络计算下一状态的最大Q值，选择相应的动作，argmax(Q(s_t+1, a_t+1) -> a_max_policy_net
        next_max_q_actions = self.policy_net(next_state_batch).max(1)[1].detach().unsqueeze(1)
        # 根据a_max_policy_net，采用目标网络计算Q值, Q(s_t+1, a_max_policy_net)
        next_q_target_values = self.target_net(next_state_batch).gather(dim=1, index=next_max_q_actions).squeeze(1)
        # 期望Q值: reward + gamma * Q_target_net(s_t+1, a_max_policy_net) * (1-done)
...
def env_agent_config(cfg):
    ...
    agent = DDQNAgent(cfg, state_dim, action_dim)
    return env, agent
