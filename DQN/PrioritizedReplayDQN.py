...
class PRDQNConfig:
    def __init__(self):
        ...
        self.capacity = 10000 # 经验回放池大小
        self.hidden_dim = 16 # 策略网络中每层隐藏层的神经单元数量
        self.start_epsilon = 0.8 # epsilon-greedy 的初始值
        self.end_epsilon = 0.01 # epsilon-greedy 的终值
        self.batch_size = 12 # 每次进行参数更新的样本量大小
        self.gamma = 1 # 折扣系数
        self.lr = 0.01 # 优化器的学习率
...
class SumTree:
    ...
    def sample(self, batch_size):
        tree_idx, batch, ISWeights = np.zeros(batch_size, dtype=np.int32), np.zeros((batch_size, self.data[0].shape[0])), np.zeros((batch_size, 1))
